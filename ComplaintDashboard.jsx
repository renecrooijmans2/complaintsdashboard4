...
/* ══════════════════════════════════════════
   THEME — "Deep Blue Ops"
   Near-black blue canvas, floating ink cards, hairline borders,
   one electric-blue accent used only where an action or a number lives.
   ══════════════════════════════════════════ */
var N = {
  /* canvas + surfaces */
  bg: "#06070B",          // page canvas
  bgC: "#0D0F16",         // card
  bgS: "#131622",         // raised surface (table head, inputs)
  elev: "#171B29",        // hover / active surface

  /* ink */
  text: "rgba(233,239,255,0.95)",
  textS: "rgba(233,239,255,0.58)",
  textT: "rgba(233,239,255,0.34)",
  ...
  orange: "#FBBF24",
  blue: "#5B94FF",
  grey: "rgba(233,239,255,0.38)",

  /* accent surfaces */
  blueDim: "rgba(91,148,255,0.13)",
  blueLine: "rgba(91,148,255,0.34)",

  /* gradients + light */
  grad: "linear-gradient(135deg,#7DB0FF 0%,#2F6BE8 100%)",
  gradSoft: "linear-gradient(160deg,rgba(125,176,255,0.14) 0%,rgba(47,107,232,0.03) 60%)",
  gradCard: "linear-gradient(160deg,rgba(255,255,255,0.045) 0%,rgba(255,255,255,0.012) 42%,rgba(255,255,255,0) 100%)",
  shadow: "0 20px 50px rgba(0,0,0,0.55)",
  glow: "0 6px 20px rgba(47,107,232,0.38)",
};
/* radii — one scale, used everywhere */
var R = { card: 18, tile: 14, btn: 11, pill: 999 };
...
function cardStyle(extra) {
  return Object.assign({
    background: N.bgC,
    backgroundImage: N.gradCard,
    border: "1px solid " + N.border,
    borderRadius: R.card,
    boxShadow: "0 1px 0 rgba(255,255,255,0.04) inset, 0 12px 30px rgba(0,0,0,0.35)",
  }, extra || {});
}
/* variant: "primary" | "ghost" | "danger" | "good" */
function btn(variant, extra) {
  var base = {
    fontFamily: "inherit", fontSize: 11.5, fontWeight: 650, letterSpacing: "-0.01em",
    padding: "8px 15px", borderRadius: R.btn, cursor: "pointer",
    display: "inline-flex", alignItems: "center", gap: 7, whiteSpace: "nowrap",
    transition: "transform .12s ease, filter .12s ease, background .12s ease",
  };
  var v = {
    primary: { background: N.grad, color: "#fff", border: "1px solid rgba(255,255,255,0.14)", boxShadow: N.glow },
    ghost:   { background: N.bgS, color: N.textS, border: "1px solid " + N.border },
    good:    { background: "rgba(52,211,153,0.12)", color: N.green, border: "1px solid rgba(52,211,153,0.32)" },
    danger:  { background: "rgba(251,113,133,0.12)", color: N.red, border: "1px solid rgba(251,113,133,0.32)" },
  }[variant] || {};
  return Object.assign(base, v, extra || {});
}
function pill(color, bg, border, extra) {
  return Object.assign({
...
var LABEL = { fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: N.textT };

/* Minimal 1.6px stroke icon set — one visual language, no emoji. */
function Icon(props) {
  var n = props.name, s = props.size || 15, c = props.color || "currentColor";
  var p = {
    ...
    stop:    "M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18zM6 6l12 12",
    alert:   "M12 3l9.5 17H2.5zM12 10v4M12 17.5v.5",
    bag:     "M4 8h16l-1.2 12H5.2zM8.5 8V6a3.5 3.5 0 0 1 7 0v2",
    flag:    "M5 21V4M5 4h11l-2 3.5L16 11H5",
    percent: "M6.5 6.5h.01M17.5 17.5h.01M19 5L5 19M6.5 4a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM17.5 15a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5z",
    fire:    "M12 3s5 4 5 8a5 5 0 0 1-10 0c0-1.6.8-2.9 1.6-3.8C9.6 8 12 6 12 3z",
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c}
      strokeWidth={props.weight || 1.7} strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
      <path d={p} />
    </svg>
  );
}

/* Gradient-tinted square that carries an icon — used on KPI tiles + nav. */
function IconTile(props) {
  var tint = props.tint || N.blue;
  return (
    <div style={{
      width: props.size || 30, height: props.size || 30, borderRadius: 10,
      display: "flex", alignItems: "center", justifyContent: "center",
      background: "linear-gradient(150deg," + hexA(tint, 0.22) + "," + hexA(tint, 0.06) + ")",
      border: "1px solid " + hexA(tint, 0.25),
      flexShrink: 0,
    }}>
      <Icon name={props.name} size={props.iconSize || 15} color={tint} />
    </div>
  );
}
function hexA(hex, a) {
...
/* Injected once — fonts, scrollbars, focus rings, native control theming. */
function GlobalStyles() {
  return (
    <style>{`
      ...
      *::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.10);border-radius:9px;border:2px solid transparent;background-clip:content-box}
      *::-webkit-scrollbar-thumb:hover{background:rgba(255,255,255,0.18);background-clip:content-box}
      button:focus-visible,select:focus-visible,input:focus-visible,textarea:focus-visible{outline:2px solid ${N.blue};outline-offset:2px}
...
function Logo(props) {
  var s = props && props.size ? props.size : 34;
  return (
    <div style={{
      width: s, height: s, borderRadius: 11, background: N.grad, boxShadow: N.glow,
      display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
    }}>
      <svg width={s * 0.58} height={s * 0.58} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 16.5l5-6 4 3 5.5-8" />
        <path d="M14.5 5.5H18V9" />
      </svg>
    </div>
  );
}
...
/* Status chip */
var STATUS_STYLES = {
  "New arrival": { color: "#FBBF24", bg: "rgba(251,191,36,0.11)", border: "rgba(251,191,36,0.30)" },
  Active:   { color: N.green,  bg: "rgba(52,211,153,0.11)",  border: "rgba(52,211,153,0.28)" },
  Edited:   { color: N.blue,   bg: N.blueDim,                border: N.blueLine },
  Inactive: { color: N.grey,   bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.10)" },
  Stopped:  { color: N.red,    bg: "rgba(251,113,133,0.10)", border: "rgba(251,113,133,0.28)" },
};
function StatusChip(props) {
  var s = STATUS_STYLES[props.status] || STATUS_STYLES.Inactive;
  return (
    <span style={pill(s.color, s.bg, s.border, { fontSize: 9, letterSpacing: "0.05em" })}>
      <span style={{ width: 5, height: 5, borderRadius: 5, background: s.color, boxShadow: "0 0 8px " + s.color, animation: props.status === "Active" ? "pulseDot 2.4s ease-in-out infinite" : "none" }} />
      {props.status}
    </span>
  );
}
        ...
        return (
          <div key={i} style={{ display: "flex", flexDirection: "column", gap: 4, padding: "8px 0", borderTop: i === 0 ? "none" : "1px solid " + N.border, opacity: a.pending ? 0.6 : 1 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "baseline", fontSize: 11.5, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 11.5, color: N.textS, background: "rgba(82,156,202,0.12)", padding: "2px 7px", borderRadius: 10, whiteSpace: "nowrap" }}>{cat ? cat.label : a.category}</span>
              ...
              {canUndo && !a.pending && onEdit && (
                <button onClick={function () { onEdit(a); }} title="Edit this log entry"
                  style={{ background: "transparent", border: "1px solid rgba(82,156,202,0.35)", color: N.blue, fontSize: 9, padding: "1px 7px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>
                  Edit
                </button>
              )}
              {canUndo && !a.pending && (
                <button onClick={function () { onUndo(a); }} title="Remove this action from the sheet"
                  style={{ background: "transparent", border: "1px solid rgba(255,115,105,0.3)", color: N.red, fontSize: 9, padding: "1px 7px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>
                  {"\u21BA"} Undo
                </button>
              )}
            </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(" + CATEGORIES.length + ", minmax(0, 1fr))", gap: 6 }}>
      {CATEGORIES.map(function (c) {
        ...
        return (
          <div key={c.key} style={{ background: getHeatBg(pct, z), border: "1px solid " + N.border, borderRadius: R.tile, padding: "9px 10px", textAlign: "center" }}>
    ...
    <div style={{ background: N.bg, border: "1px solid " + N.border, borderRadius: 10, padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>
      ...
      {!d || d.totalComplaints === 0 ? (
        ...
        <>
          ...
          <div style={{ background: "rgba(82,156,202,0.08)", border: "1px solid rgba(82,156,202,0.3)", borderRadius: 9, padding: "8px 10px" }}>
...
function FocusPanel(props) {
  ...
  var inputStyle = { background: N.bgS, border: "1px solid " + N.border, borderRadius: R.btn, color: N.text, fontSize: 11.5, fontFamily: "inherit", padding: "8px 11px", outline: "none", width: "100%" };
  var btnStyle = function (color, bg, border) {
    return { background: bg, border: "1px solid " + border, color: color, fontSize: 11, fontWeight: 650, padding: "7px 14px", borderRadius: R.btn, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 6 };
  };

  return (
    <div style={cardStyle({ background: N.bgC, padding: 18, display: "flex", flexDirection: "column", gap: 14, position: "relative", zIndex: 40, boxShadow: N.shadow })}>

      <div style={{ fontSize: 16, fontWeight: 750, letterSpacing: "-0.03em", color: N.text, display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
        ...
        {props.onClose && (
          <button onClick={props.onClose} style={btn("ghost", { fontSize: 10.5, padding: "5px 12px" })}>Exit (Esc)</button>
        )}
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            {row.scaleRisk && (
              <span title={"Orders just ramped (" + row.scaleRisk.recent2 + " last 2 wks vs " + row.scaleRisk.prev2 + " before). " + row.scaleRisk.recentComplaints + " complaints (" + row.scaleRisk.recentSerious + " serious) in the last 2 wks. With ~2-week shipping, the full complaint wave lands ~2 weeks after scaling."}
                style={{ fontSize: 9, fontWeight: 700, color: N.red, background: "rgba(255,115,105,0.15)", border: "1px solid rgba(255,115,105,0.4)", padding: "2px 8px", borderRadius: 10, letterSpacing: "0.02em" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(" + CATEGORIES.length + ", minmax(0, 1fr))", gap: 6 }}>
            {[
              ...
              return (
                <div key={s.label} style={{ background: N.bgS, border: "1px solid " + N.border, borderRadius: R.tile, padding: "9px 10px", textAlign: "center" }}>
                  <div style={{ fontSize: 8.5, color: N.textT, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.label}</div>
                  <div style={{ fontSize: 15, fontWeight: 750, letterSpacing: "-0.03em", color: s.color || N.text, fontVariantNumeric: "tabular-nums", marginTop: 2 }}>{s.value}</div>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ borderTop: "1px solid " + N.border, paddingTop: 10 }}>
        ...
        {showStopForm && (
          <div style={{ background: N.bg, border: "1px solid rgba(255,115,105,0.25)", borderRadius: 10, padding: 12, marginBottom: 10, display: "flex", gap: 8, alignItems: "end" }}>
            <div style={{ flex: 1 }}>
...
function FloatingWidgets(props) {
  ...
  var panelBase = { position: "fixed", right: 14, bottom: 64, width: 400, maxWidth: "90vw", maxHeight: "72vh", background: "rgba(13,15,22,0.97)", backdropFilter: "blur(16px)", border: "1px solid " + N.borderS, borderRadius: R.card, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: N.shadow, zIndex: 50, animation: "rise .2s ease" };
  var fabStyle = function (active) {
    return active
      ? btn("primary", { fontSize: 11.5, padding: "8px 15px" })
      : btn("ghost", { fontSize: 11.5, padding: "8px 15px", background: "rgba(19,22,34,0.92)", backdropFilter: "blur(10px)", border: "1px solid " + N.borderS });
  };
        ...
        <div style={panelBase}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 8, minHeight: 180 }}>
            ...
            {msgs.map(function (m, i) {
              return (
                <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%", background: m.role === "user" ? "rgba(82,156,202,0.15)" : N.bg, border: "1px solid " + N.border, borderRadius: 10, padding: "5px 7px", fontSize: 11, color: N.text, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
                  {m.content}
                </div>
              );
            })}
        ...
        <div style={panelBase}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 8, minHeight: 140 }}>
            ...
            {fbMsgs.map(function (m, i) {
              return (
                <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%", background: m.role === "user" ? "rgba(82,156,202,0.15)" : N.bg, border: "1px solid " + N.border, borderRadius: 10, padding: "7px 10px", fontSize: 11, color: N.text, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
                  {m.content}
                </div>
              );
            })}
...
export default function ComplaintDashboard() {
  ...
  // Counts per status for the left rail — same filters as the table, minus the status filter itself.
  var statusCounts = useMemo(function () {
    var c = { all: 0, active: 0, "new arrival": 0, edited: 0, inactive: 0, stopped: 0 };
    productData.forEach(function (p) {
      var t = p.product || "";
      for (var i = 0; i < EXCLUDE_TITLE_PATTERNS.length; i++) { if (EXCLUDE_TITLE_PATTERNS[i].test(t)) return; }
      if (p.orders < minSales && p.status !== "New arrival") return;
      c.all++;
      var k = (p.status || "").toLowerCase();
      if (c[k] != null) c[k]++;
    });
    return c;
  }, [productData, minSales]);
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: N.bg, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 18, fontFamily: FONT }}>
        <GlobalStyles />
        <div style={{ position: "relative", width: 46, height: 46 }}>
          <div style={{ position: "absolute", inset: 0, border: "2px solid " + N.border, borderTopColor: N.blue, borderRadius: "50%", animation: "spin 0.9s cubic-bezier(.5,.1,.5,.9) infinite" }} />
          <div style={{ position: "absolute", inset: 12, borderRadius: 8, background: N.grad, opacity: 0.85, filter: "blur(0.2px)" }} />
        </div>
        <div style={{ color: N.textS, fontSize: 12, fontWeight: 600, letterSpacing: "-0.01em" }}>Loading complaint data</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: N.bg, color: N.text, fontFamily: FONT, display: "flex", zoom: UI_ZOOM, position: "relative" }}>
      <GlobalStyles />
      {/* Ambient light — one soft blue bloom behind the top-left, the only decoration on the page */}
      <div aria-hidden style={{ position: "fixed", top: -180, left: 40, width: 620, height: 420, background: "radial-gradient(closest-side, rgba(47,107,232,0.18), transparent)", filter: "blur(20px)", pointerEvents: "none", zIndex: 0 }} />

      {/* ═══ LEFT RAIL — nav IS the status filter, with live counts ═══ */}
      <aside style={{
        width: 196, flexShrink: 0, borderRight: "1px solid " + N.border,
        background: "rgba(9,11,17,0.6)", backdropFilter: "blur(14px)",
        position: "sticky", top: 0, alignSelf: "flex-start", height: "100vh",
        display: "flex", flexDirection: "column", padding: "18px 12px 14px", zIndex: 2,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 6px 18px" }}>
          <Logo size={32} />
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 750, letterSpacing: "-0.02em", lineHeight: 1.1 }}>Complaints</div>
            <div style={{ fontSize: 9.5, color: N.textT, letterSpacing: "0.02em" }}>{STORE_CSVS[selectedStore].name}</div>
          </div>
        </div>

        <div style={Object.assign({ padding: "0 6px 8px" }, LABEL)}>Review queue</div>
        <nav style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {[
            { key: "active", label: "Needs review", icon: "pulse", tint: N.green },
            { key: "new arrival", label: "New arrivals", icon: "sparkle", tint: N.orange },
            { key: "edited", label: "Edited", icon: "edit", tint: N.blue },
            { key: "all", label: "All products", icon: "grid", tint: N.blue },
            { key: "inactive", label: "Inactive", icon: "moon", tint: N.grey },
            { key: "stopped", label: "Stopped", icon: "stop", tint: N.red },
          ].map(function (it) {
            var on = statusFilter === it.key;
            return (
              <button key={it.key} onClick={function () { setStatusFilter(it.key); }}
                style={{
                  display: "flex", alignItems: "center", gap: 10, width: "100%", textAlign: "left",
                  background: on ? "linear-gradient(90deg," + hexA(it.tint === N.grey ? "#5B94FF" : it.tint, 0.16) + ",rgba(255,255,255,0.02))" : "transparent",
                  border: "1px solid " + (on ? hexA(it.tint === N.grey ? "#5B94FF" : it.tint, 0.22) : "transparent"),
                  borderRadius: 11, padding: "8px 10px", cursor: "pointer", fontFamily: "inherit",
                  color: on ? N.text : N.textS, fontSize: 11.5, fontWeight: on ? 700 : 550,
                  transition: "background .15s, color .15s",
                }}>
                <Icon name={it.icon} size={14} color={on ? it.tint : N.textT} />
                <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.label}</span>
                <span style={{
                  fontSize: 9.5, fontWeight: 700, fontVariantNumeric: "tabular-nums",
                  color: on ? it.tint : N.textT,
                  background: on ? hexA(it.tint === N.grey ? "#5B94FF" : it.tint, 0.14) : "rgba(255,255,255,0.05)",
                  borderRadius: R.pill, padding: "1px 7px",
                }}>{statusCounts[it.key] != null ? statusCounts[it.key] : 0}</span>
              </button>
            );
          })}
        </nav>
        ...
        {/* Weekly progress — the one number that tells Amber if she's done */}
        {heatmapData.length > 0 && (function () {
          var done = Object.keys(checkedState.products).filter(function (p) { return heatmapData.some(function (r) { return r.key === p; }); }).length;
          var pctDone = Math.round((done / heatmapData.length) * 100);
          return (
            <div style={cardStyle({ padding: "11px 12px", borderRadius: R.tile })}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 7 }}>
                <span style={LABEL}>This week</span>
                <span style={{ fontSize: 11, fontWeight: 750, color: pctDone === 100 ? N.green : N.text, fontVariantNumeric: "tabular-nums" }}>{done}/{heatmapData.length}</span>
              </div>
              <div style={{ height: 5, borderRadius: 5, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
                <div style={{ width: pctDone + "%", height: "100%", borderRadius: 5, background: pctDone === 100 ? N.green : N.grad, transition: "width .35s ease" }} />
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                <span style={{ fontSize: 9, color: N.textT }}>Resets Monday</span>
                <button onClick={resetChecked} style={{ background: "transparent", border: "none", color: N.textT, fontSize: 9, fontWeight: 650, cursor: "pointer", fontFamily: "inherit", padding: 0 }}>Reset</button>
              </div>
            </div>
          );
        })()}
        ...
        {dataSrc === "demo" && (
          <div style={{ marginTop: 10, textAlign: "center" }}>
            <span style={pill(N.orange, "rgba(251,191,36,0.10)", "rgba(251,191,36,0.28)")}>demo data</span>
          </div>
        )}
      </aside>

      {/* ═══ MAIN COLUMN ═══ */}
      <main style={{ flex: 1, minWidth: 0, padding: "18px 22px 24px", display: "flex", flexDirection: "column", gap: 12, position: "relative", zIndex: 1 }}>

        {/* Top bar */}
        <div style={Object.assign({ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }, dimUI)}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
            <div>
              <h1 style={{ fontSize: 21, fontWeight: 780, margin: 0, letterSpacing: "-0.035em" }}>
                {statusFilter === "all" ? "All products" : statusFilter === "active" ? "Needs review" : statusFilter === "new arrival" ? "New arrivals" : statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)}
              </h1>
              <div style={{ fontSize: 10.5, color: N.textT, marginTop: 2 }}>
                Week {weekRange[0]}{"\u2013"}{weekRange[1]} {"\u00B7"} {visibleProducts} of {totalProducts} products
              </div>
            </div>
            {/* Store picker */}
            <div style={{ display: "flex", gap: 3, background: N.bgS, borderRadius: 13, padding: 3, border: "1px solid " + N.border }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            {/* Week range */}
            <div style={{ display: "flex", alignItems: "center", gap: 5, background: N.bgS, borderRadius: R.btn, padding: "6px 11px", border: "1px solid " + N.border }}>
              <Icon name="clock" size={13} color={N.textT} />
              <select value={weekRange[0]} onChange={function (e) { setWeekRange([Number(e.target.value), Math.max(Number(e.target.value), weekRange[1])]); }} style={{ background: "transparent", border: "none", color: N.text, fontSize: 11.5, fontWeight: 600, fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
                {availableWeeks.map(function (w) { return <option key={w} value={w}>W{w}</option>; })}
              </select>
              <span style={{ color: N.textT, fontSize: 10 }}>{"\u2192"}</span>
              <select value={weekRange[1]} onChange={function (e) { setWeekRange([Math.min(weekRange[0], Number(e.target.value)), Number(e.target.value)]); }} style={{ background: "transparent", border: "none", color: N.text, fontSize: 11.5, fontWeight: 600, fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
                {availableWeeks.map(function (w) { return <option key={w} value={w}>W{w}</option>; })}
              </select>
            </div>
            {/* Min sales */}
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: N.bgS, borderRadius: R.btn, padding: "6px 11px", border: "1px solid " + N.border }}>
              <Icon name="bag" size={13} color={N.textT} />
              <span style={{ fontSize: 10, color: N.textT, fontWeight: 600 }}>Min</span>
              <input type="number" min={0} step={10} value={minSales}
                onChange={function (e) { setMinSales(Math.max(0, Number(e.target.value) || 0)); }}
                style={{ background: "transparent", border: "none", color: N.text, fontSize: 11.5, fontWeight: 650, fontFamily: "inherit", outline: "none", width: 42 }} />
              <span style={{ fontSize: 10, color: N.textT }}>orders</span>
            </div>
            <button onClick={runAllRecommendations} disabled={runState.running}
              style={btn(runState.running ? "ghost" : "primary", { cursor: runState.running ? "default" : "pointer", opacity: runState.running ? 0.75 : 1 })}>
              <Icon name={runState.running ? "refresh" : "sparkle"} size={14} color={runState.running ? N.textS : "#fff"} />
        ...
        {/* KPI strip */}
        <div style={Object.assign({ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1.7fr", gap: 10 }, dimUI)}>
          {[
            { label: "Total orders", value: totals.orders.toLocaleString(), icon: "bag", tint: N.blue },
            { label: "Complaints", value: totals.complaints.toLocaleString(), icon: "alert", tint: N.orange },
            { label: "Complaint rate", value: fmtPct(totals.pct), icon: "percent", tint: totals.pct >= KILL_DEBATE_THRESHOLD ? N.red : N.green },
            { label: "Worst product", value: totals.worst ? totals.worst.product : "\u2014", sub: totals.worst ? fmtPct(totals.worst.pct) + " complaint rate" : "", icon: "fire", tint: N.red, wide: true },
          ].map(function (k) {
            return (
              <div key={k.label} style={cardStyle({ padding: "13px 15px", display: "flex", flexDirection: "column", gap: 9, minWidth: 0 })}>
                <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                  <IconTile name={k.icon} tint={k.tint} size={28} iconSize={14} />
                  <span style={LABEL}>{k.label}</span>
                </div>
                <div style={{
                  fontSize: k.wide ? 15 : 25, fontWeight: 750, color: N.text, letterSpacing: "-0.035em",
                  fontVariantNumeric: "tabular-nums", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.15,
                }}>{k.value}</div>
                {k.sub && <div style={{ fontSize: 10.5, color: k.tint, fontWeight: 600, marginTop: -4 }}>{k.sub}</div>}
              </div>
            );
          })}
        </div>
        ...
        {loadErr && !loading && (
          <div style={{ background: "rgba(251,113,133,0.08)", border: "1px solid rgba(251,113,133,0.28)", borderRadius: R.tile, padding: "10px 15px", fontSize: 11.5, color: N.red }}>
            {loadErr}
          </div>
        )}

        {/* Weekly workflow — a real sequence, so it gets real numbers */}
        <div style={Object.assign(cardStyle({ padding: "13px 15px" }), dimUI)}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <Icon name="flag" size={13} color={N.blue} />
            <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: "-0.01em" }}>Weekly review</span>
            <span style={{ fontSize: 10, color: N.textT }}>every product, once a week</span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 10 }}>
            {[
              { n: "1", t: "Needs review", d: "Top to bottom, most complaints first. Open each product, read the AI analysis, act." },
              { n: "2", t: "New arrivals", d: "Fix fresh products before they scale \u2014 1\u20132 early complaints is already a signal. Red tags first." },
              { n: "3", t: "Edited", d: "Check back on the edits you made and anything still open (see Last edit)." },
              { n: "4", t: "All stores", d: "Switch store in the top bar and repeat the same three passes." },
            ].map(function (s) {
              return (
                <div key={s.n} style={{ display: "flex", gap: 9, minWidth: 0 }}>
                  <span style={{
                    width: 19, height: 19, flexShrink: 0, borderRadius: 7, background: N.blueDim, border: "1px solid " + N.blueLine,
                    color: N.blue, fontSize: 9.5, fontWeight: 750, display: "flex", alignItems: "center", justifyContent: "center",
                  }}>{s.n}</span>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: N.text, marginBottom: 2 }}>{s.t}</div>
                    <div style={{ fontSize: 10, color: N.textS, lineHeight: 1.5 }}>{s.d}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div style={cardStyle({ padding: 0, position: "relative", overflow: "hidden" })}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px", borderBottom: "1px solid " + N.border, flexWrap: "wrap", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <span style={{ fontSize: 12.5, fontWeight: 700, letterSpacing: "-0.02em" }}>Products</span>
              <span style={{ fontSize: 10, color: N.textT, fontVariantNumeric: "tabular-nums" }}>{heatmapData.length} shown</span>
            </div>
            {focusedProduct && (
              <button onClick={closeFocus} style={btn("ghost", { fontSize: 10.5, padding: "5px 12px" })}>
                <Icon name="x" size={12} color={N.textS} /> Exit focus
              </button>
            )}
          </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", tableLayout: "fixed", borderCollapse: "collapse", fontSize: 11.5, fontVariantNumeric: "tabular-nums" }}>
            <thead>
              <tr>
                <th style={{ background: N.bgS, padding: "9px 4px", position: "sticky", top: 0, width: 30, borderBottom: "1px solid " + N.border }}></th>
                {[
                  ...
                  return (
                    <th key={i} style={{ background: N.bgS, color: N.textT, fontWeight: 700, fontSize: 9, textTransform: "uppercase", letterSpacing: "0.09em", padding: "9px 9px", textAlign: h.align, whiteSpace: "nowrap", position: "sticky", top: 0, width: h.w || "auto", borderBottom: "1px solid " + N.border }}>
                      {h.label}
                    </th>
                  );
                })}
            ...
            <tbody>
              {heatmapData.length === 0 && (
                <tr>
                  <td colSpan={visibleColCount} style={{ padding: "38px 24px", textAlign: "center", color: N.textS, fontSize: 12, lineHeight: 1.6 }}>
                    <div style={{ fontWeight: 700, color: N.text, marginBottom: 4 }}>Nothing here yet</div>
                    Lower the min sales threshold, widen the week range, or pick another queue on the left.
                  </td>
                </tr>
              )}
              {heatmapData.map(function (row, ri) {
                ...
                var rowEls = [
                  <tr
                    ...
                    style={{
                      background: isFocused ? N.blueDim : (isHovered ? "rgba(255,255,255,0.035)" : "transparent"),
                      boxShadow: row.killSignal && row.killSignal.tier === "auto"
                        ? "inset 2px 0 0 " + N.red
                        : (row.scaleRisk ? "inset 2px 0 0 " + N.orange : (isFocused ? "inset 2px 0 0 " + N.blue : "none")),
                      opacity: dimmed ? 0.1 : (isChecked ? 0.24 : 1),
                      filter: dimmed ? "blur(2.5px) brightness(0.45)" : "none",
                      transition: "opacity 0.2s, filter 0.2s, background 0.15s",
                    }}
                  >
                    <td style={{ padding: "0", textAlign: "center", borderBottom: "1px solid " + N.border, width: 30 }}>
                    ...
                    <td style={{ padding: "9px 9px", textAlign: "left", color: N.textS, borderBottom: "1px solid " + N.border, whiteSpace: "nowrap", fontWeight: 600 }}>
                      {row.firstWeek != null ? "W" + row.firstWeek : "\u2014"}
                    </td>
                    <td style={{ padding: "9px 9px", textAlign: "left", borderBottom: "1px solid " + N.border }}>
                      <StatusChip status={row.status} />
                    </td>
                    <td style={{ padding: "9px 9px", textAlign: "left", color: row.orders7d != null && row.orders7d < INACTIVE_SALES_7D ? N.textT : N.textS, borderBottom: "1px solid " + N.border, fontWeight: 600 }}>
                      {row.orders7d != null ? row.orders7d.toLocaleString() : "\u2014"}
                    </td>
                    <td style={{ padding: "9px 9px", textAlign: "left", fontWeight: 750, fontSize: 12.5, letterSpacing: "-0.02em", borderBottom: "1px solid " + N.border, color: row.pct >= KILL_TOTAL_THRESHOLD ? N.red : N.text, background: row.pct >= KILL_TOTAL_THRESHOLD ? "rgba(251,113,133,0.10)" : "transparent" }}>
                      {row.sampleSmall ? row.complaints + "\u00D7" : fmtPct(row.pct)}
                    </td>
                    <td style={{ padding: "9px 9px", textAlign: "left", color: N.textS, borderBottom: "1px solid " + N.border, fontVariantNumeric: "tabular-nums", fontWeight: 600 }}>
                      {(function () {
                    ...
                    <td style={{ padding: "9px 9px", textAlign: "left", borderBottom: "1px solid " + N.border, whiteSpace: "nowrap" }}>
                      {row.killSignal ? (
                        <span title={(row.killSignal.tier === "auto" ? "AUTO KILL \u2014 " : "DEBATE \u2014 ") + row.killSignal.reasons.join(", ")}
                          style={row.killSignal.tier === "auto"
                            ? pill(N.red, "rgba(251,113,133,0.13)", "rgba(251,113,133,0.35)")
                            : pill(N.textS, "rgba(255,255,255,0.05)", N.border)}>
                        ...
                        <span title={"Orders just ramped (" + row.scaleRisk.recent2 + " last 2 wks vs " + row.scaleRisk.prev2 + " before) and early feedback is negative (" + row.scaleRisk.recentComplaints + " complaints, " + row.scaleRisk.recentSerious + " serious, in the last 2 wks). With ~2-week shipping, the full complaint wave lands ~2 weeks after scaling \u2014 watch closely."}
                          style={pill(N.orange, "rgba(251,191,36,0.13)", "rgba(251,191,36,0.35)")}>
                          SCALE RISK
                        </span>
                      ) : row.earlyWarning ? (
                        <span title={row.earlyWarning.direction === "both"
                          ...
                          style={pill(N.textS, "rgba(255,255,255,0.05)", N.border)}>
                    ...
                    <td
                      ...
                      style={{ padding: "9px 9px", textAlign: "left", color: N.text, fontWeight: 600, borderBottom: "1px solid " + N.border, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textDecoration: isChecked ? "line-through" : "none", cursor: "pointer" }}>
                      <span style={{ color: isStopped ? N.textS : N.text, borderBottom: "1px dotted rgba(255,255,255,0.18)", paddingBottom: 1 }}>{row.product}</span>
                                          </td>
                    {statusFilter !== "active" && <td style={{ padding: "9px 9px", textAlign: "left", color: N.textS, borderBottom: "1px solid " + N.border, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 10.5, maxWidth: 240 }}
                      title={(function () {
      ...
      {/* Footer */}
      <div style={{ padding: "10px 2px 0", borderTop: "1px solid " + N.border, display: "flex", justifyContent: "space-between", gap: 16, fontSize: 9.5, color: N.textT, lineHeight: 1.6 }}>
        <span style={{ whiteSpace: "nowrap" }}>Complaint Tracker {"\u2014"} {title}</span>
        <span style={{ textAlign: "right" }}>{visibleProducts}/{totalProducts} products {"\u00B7"} {Object.keys(stoppedAds).length} stopped {"\u00B7"} Complaints W{weekRange[0]}{"\u2013"}W{weekRange[1]} vs Orders W{ordersWR[0]}{"\u2013"}W{ordersWR[1]} {"\u00B7"} 14d lag {"\u00B7"} Orders 14d = W{latestDataWeek - 1}{"\u2013"}W{latestDataWeek} {"\u00B7"} Min {minSales} sales {"\u00B7"} Margin data: {Object.keys(contribData).length} products{Object.keys(contribData).length === 0 ? " \u2014 check contribUrl gid (README)" : ""}</span>
      </div>
      </main>

      {toast && createPortal(
        <div style={{ position: "fixed", bottom: 26, left: "50%", transform: "translateX(-50%)", background: "rgba(19,22,34,0.97)", backdropFilter: "blur(14px)", border: "1px solid " + N.blueLine, color: N.text, fontSize: 14, fontWeight: 650, letterSpacing: "-0.01em", padding: "13px 24px", borderRadius: R.tile, zIndex: 10000, boxShadow: N.shadow, animation: "toastIn 0.25s ease", fontFamily: FONT, whiteSpace: "nowrap", maxWidth: "90vw", overflow: "hidden", textOverflow: "ellipsis" }}>
      ...
      {focusedProduct && (
        <div onClick={closeFocus}
          style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(3,4,7,0.86)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", zIndex: 30, opacity: closing ? 0 : 1, transition: "opacity 0.3s ease" }} />
      )}
