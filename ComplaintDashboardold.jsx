...
var N = {
  ...
  bgS: "#16141C",
  bgC: "#121016",
  text: "rgba(245,243,248,0.92)",
  textS: "rgba(245,243,248,0.55)",
  textT: "rgba(245,243,248,0.32)",
  border: "rgba(245,243,248,0.08)",
  green: "#4ADE80",
  red: "#F87171",
  orange: "#FBBF24",
  blue: "#3B82F6",
  grey: "rgba(245,243,248,0.38)",
};
...
function StatusChip(props) {
  ...
  return (
    <span style={{ fontSize: 9, fontWeight: 700, color: s.color, background: s.bg, border: "1px solid " + s.border, padding: "2px 7px", borderRadius: 10, letterSpacing: "0.03em", textTransform: "uppercase", whiteSpace: "nowrap" }}>
      {props.status}
    </span>
  );
}
...
function FocusPanel(props) {
  ...
  return (
    <div style={{ background: N.bg, border: "1px solid " + N.border, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 14, position: "relative", zIndex: 40 }}>
...
function FloatingWidgets(props) {
  ...
  var panelBase = { position: "fixed", right: 14, bottom: 60, width: 400, maxWidth: "90vw", maxHeight: "72vh", background: N.bgC, border: "1px solid " + N.border, borderRadius: 12, display: "flex", flexDirection: "column", boxShadow: "0 8px 32px rgba(0,0,0,0.55)", zIndex: 50 };
    ...
    <div style={{ minHeight: "100vh", background: N.bg, color: N.text, fontFamily: FONT, padding: "14px 18px", display: "flex", flexDirection: "column", gap: 10, zoom: UI_ZOOM }}>
      ...
      <div style={Object.assign({ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }, dimUI)}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 4, background: N.bgC, borderRadius: 10, padding: 4, border: "1px solid " + N.border }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <button onClick={runAllRecommendations} disabled={runState.running}
            style={{ background: runState.running ? "rgba(52,211,153,0.12)" : "rgba(52,211,153,0.16)", border: "1px solid rgba(52,211,153,0.45)", color: N.green, fontSize: 11, fontWeight: 700, padding: "6px 14px", borderRadius: 9, cursor: runState.running ? "default" : "pointer", fontFamily: "inherit" }}>
            {runState.running ? "Running " + runState.done + "/" + runState.total + "\u2026" : "Run all recommendations"}
          </button>
          {/* Week range */}
          <div style={{ display: "flex", alignItems: "center", gap: 4, background: N.bgC, borderRadius: 12, padding: "4px 10px", border: "1px solid " + N.border }}>
          ...
          {/* Min sales */}
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: N.bgC, borderRadius: 12, padding: "4px 10px", border: "1px solid " + N.border }}>
      ...
      <div style={Object.assign({ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1.5fr", gap: 8 }, dimUI)}>
        {[
          ...
          return (
            <div key={k.label} style={{ background: N.bgC, border: "1px solid " + N.border, borderRadius: 10, padding: "10px 14px" }}>
      ...
      {loadErr && !loading && (
        <div style={{ background: "rgba(255,115,105,0.08)", border: "1px solid rgba(255,115,105,0.3)", borderRadius: 10, padding: "8px 14px", fontSize: 11, color: N.red }}>
          {loadErr}
        </div>
      )}

      {/* Amber's weekly workflow */}
      <div style={Object.assign({ background: N.bgC, border: "1px solid " + N.border, borderRadius: 10, padding: "10px 14px", display: "flex", flexDirection: "column", gap: 4 }, dimUI)}>
      ...
      {/* Table */}
      <div style={{ background: N.bgC, border: "1px solid " + N.border, borderRadius: 12, padding: 12, position: "relative" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            {heatmapData.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 10, color: N.textS }}>
                ...
                <button onClick={resetChecked} style={{ background: "transparent", border: "1px solid " + N.border, color: N.textT, fontSize: 9, padding: "2px 8px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit" }}>
                  Reset
                </button>
              </div>
            )}
            {focusedProduct && (
              <button onClick={closeFocus} style={{ background: "transparent", border: "1px solid " + N.border, color: N.textS, fontSize: 10, fontWeight: 600, padding: "3px 10px", borderRadius: 12, cursor: "pointer", fontFamily: "inherit" }}>
                {"\u2715"} Exit focus
              </button>
            )}
          </div>
                      ...
                      {row.killSignal ? (
                        <span title={(row.killSignal.tier === "auto" ? "AUTO KILL \u2014 " : "DEBATE \u2014 ") + row.killSignal.reasons.join(", ")}
                          style={row.killSignal.tier === "auto"
                            ? { fontSize: 9, fontWeight: 700, color: N.red, background: "rgba(255,115,105,0.15)", border: "1px solid rgba(255,115,105,0.4)", padding: "2px 7px", borderRadius: 10, letterSpacing: "0.02em" }
                        ...
                        <span title={"Orders just ramped (" + row.scaleRisk.recent2 + " last 2 wks vs " + row.scaleRisk.prev2 + " before) and early feedback is negative (" + row.scaleRisk.recentComplaints + " complaints, " + row.scaleRisk.recentSerious + " serious, in the last 2 wks). With ~2-week shipping, the full complaint wave lands ~2 weeks after scaling \u2014 watch closely."}
                          style={{ fontSize: 9, fontWeight: 700, color: N.red, background: "rgba(255,115,105,0.15)", border: "1px solid rgba(255,115,105,0.4)", padding: "2px 7px", borderRadius: 10, letterSpacing: "0.02em" }}>
                          SCALE RISK
                        </span>
                      ) : row.earlyWarning ? (
